...
import { BarChart2, ChevronDown, ChevronUp, Edit, Filter, Plus, Save, Search, Trash2, X, Languages } from "lucide-react";
...
const AdminPanel = () => {
  ...
  const [editLanguage, setEditLanguage] = useState<'hr' | 'en' | 'de' | null>(null);
...
const saveItem = async () => {
  ...
  const getStats = () => {
    ...
    const categoriesCount = new Set(menuItems.map((item) => item.collection)).size
  ...
  const handleRealtimeUpdate = (payload: any) => {
    switch (payload.eventType) {
      ...
      case 'UPDATE':
        setMenuItems(prev => 
          prev.map(item => item.external_id === payload.new.external_id ? payload.new : item)
        );
        break;
      case 'DELETE':
        setMenuItems(prev => 
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          {isAddingNew && (
            <div className="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow-md">
              <div className="mb-4 flex items-center justify-between">
                ...
                <button onClick={cancelEditing} className="rounded-full p-1 hover:bg-gray-100">
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateEditingField("collection", e.target.value)}
                    >
                      {availableCollections.map((category) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateEditingField("price", Number.parseFloat(e.target.value) || 0)}
                    />
                  </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateEditingField("product_name", e.target.value)}
                        />
                      </div>
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => updateEditingField("description_hr", e.target.value)}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  {translationAvailable && (
                    <div className="flex justify-center">
                      ...
                      >
                        ...
                        {isTranslating ? "Translating..." : "Auto-translate to other languages"}
                      </button>
                    </div>
                  )}

                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <h3 className="mb-3 font-semibold">English (EN)</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="mb-1 block font-medium">Name</label>
                        <input
                          type="text"
                          className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-[#C41E3A] focus:outline-none focus:ring-2 focus:ring-[#C41E3A]/50"
                          value={editingItem!.product_name_en || ""}
                          onChange={(e) => updateEditingField("product_name_en", e.target.value)}
                        />
                      </div>
                      <div>
                        <label className="mb-1 block font-medium">Description</label>
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                    <h3 className="mb-3 font-semibold">German (DE)</h3>
                    <div className="space-y-4">
                      <div>
                        <label className="mb-1 block font-medium">Name</label>
                        <input
                          type="text"
                          className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-[#C41E3A] focus:outline-none focus:ring-2 focus:ring-[#C41E3A]/50"
                          value={editingItem!.product_name_de || ""}
                          onChange={(e) => updateEditingField("product_name_de", e.target.value)}
                        />
                      </div>
                      <div>
                        <label className="mb-1 block font-medium">Description</label>
                        <textarea
                          className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-[#C41E3A] focus:outline-none focus:ring-2 focus:ring-[#C41E3A]/50"
                          rows={2}
                          value={editingItem!.description_de || ""}
                          onChange={(e) => updateEditingField("description_de", e.target.value)}
                        />
                      </div>
                    </div>
                  </div>
                ...
                <thead className="bg-gray-50">
                  <tr>
                    <th className="whitespace-nowrap border-b px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                      <div className="flex cursor-pointer items-center" onClick={() => requestSort("product_name")}>
                        Name
                        {getSortIndicator("product_name")}
                      </div>
                    </th>
                    <th className="whitespace-nowrap border-b px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                      <div className="flex cursor-pointer items-center" onClick={() => requestSort("collection")}>
                        Category
                        {getSortIndicator("collection")}
                      </div>
                    </th>
                    <th className="whitespace-nowrap border-b px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                      <div className="flex cursor-pointer items-center" onClick={() => requestSort("price")}>
                        Price
                  ...
                  {filteredItems && filteredItems.length === 0 ? (
                    ...
                    filteredItems?.map((item) => (
                      <React.Fragment key={item.external_id}>
                        ...
                        <td className="whitespace-nowrap px-6 py-4 font-medium">{item.price.toFixed(2)} €</td>
                        <td className="whitespace-nowrap px-6 py-4">{item.size || "Regular"}</td>
      ...
      <div className=" col-span-2 flex gap-4">
          {/* English */}
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4  min-w-[45%] w-full">
            ...
            {editLanguage === 'en' ? (
              ...
              <div className="space-y-2">
                <p className="font-medium">{editingItem.product_name_en}</p>
                <p className="text-gray-600">{editingItem.description_en}</p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 min-w-[45%] w-full">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold">German (DE)</h3>
              <button
            ...
            {editLanguage === 'de' ? (
              ...
              <div className="space-y-2">
                <p className="font-medium">{editingItem.product_name_de}</p>
                <p className="text-gray-600">{editingItem.description_de}</p>
              </div>
            )}
          </div>
        </div>
